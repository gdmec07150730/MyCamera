...
public class AlbumActivity extends AppCompatActivity {
    ...
    public String [] loadAlbum(){
        ...
        Vector<Bitmap> filenName=new Vector<Bitmap>();
        if(file.exists()&&file.isDirectory()){
            String [] str=file.list();
            for(String s:str){
                if(new File(pathName+"/"+s).isFile()){
                    filenName.addElement(loadImage(pathName+"/"+s));


                }

            }
            mBgList=filenName.toArray(new Bitmap[]{});
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sel=new SensorEventListener() {
            ...
            public void onSensorChanged(SensorEvent event) {
                ...
                if(x>10&&System.currentTimeMillis()>startTime+100){
